...
if [ "${COPYFULL:-false}" == "true" ] ; then
  if [ "${USERUCIO:-false}" == "true" ] ; then
    ...
    python $SCRIPT_DIR/register_to_rucio.py -f "${FULL_TEMP}/${TASKNAME}.rntuple.edm4hep.root" -d "/${FULL_DIR}/${TASKNAME}.edm4hep.root" -s epic -r EIC-XRD
  else
    # Token for write authentication
    export BEARER_TOKEN=$(cat ${_CONDOR_CREDS:-.}/eic.use)
    if [ -n ${XRDWURL} ] ; then
...
if [ "${COPYRECO:-false}" == "true" ] ; then
  if [ "${USERUCIO:-false}" == "true" ] ; then
    ...
    python $SCRIPT_DIR/register_to_rucio.py -f "${RECO_TEMP}/${TASKNAME}.rntuple.eicrecon.edm4eic.root" -d "/${RECO_DIR}/${TASKNAME}.eicrecon.edm4eic.root" -s epic -r EIC-XRD
  else
    # Token for write authentication
    export BEARER_TOKEN=$(cat ${_CONDOR_CREDS:-.}/eic.use)
    if [ -n ${XRDWURL} ] ; then
