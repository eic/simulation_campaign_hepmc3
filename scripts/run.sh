...
mkdir -p ${INPUT_DIR}
TAG=${DETECTOR_VERSION:-main}/${DETECTOR_CONFIG}/${TAG_PREFIX:+${TAG_PREFIX}/}${TAG}${TAG_SUFFIX:+/${TAG_SUFFIX}}
...
if [ "${COPYFULL:-false}" == "true" ] ; then
  if [ "${USERUCIO:-false}" == "true" ] ; then
    RUCIO_CONFIG=/opt/campaigns/hepmc3/scripts/rucio.cfg python /opt/campaigns/hepmc3/scripts/register_to_rucio.py -f "${FULL_TEMP}/${TASKNAME}.edm4hep.root" -d "/${FULL_DIR}/${TASKNAME}.edm4hep.root" -s epic
  else
...
if [ "${COPYRECO:-false}" == "true" ] ; then
  if [ "${USERUCIO:-false}" == "true" ] ; then
    RUCIO_CONFIG=/opt/campaigns/hepmc3/scripts/rucio.cfg python /opt/campaigns/hepmc3/scripts/register_to_rucio.py -f "${RECO_TEMP}/${TASKNAME}.eicrecon.tree.edm4eic.root" -d "/${RECO_DIR}/${TASKNAME}.eicrecon.tree.edm4eic.root" -s epic
  else
    # Token for write authentication
    export BEARER_TOKEN=$(cat ${_CONDOR_CREDS}/eic.use)
    if [ -n ${XRDWURL} ] ; then
